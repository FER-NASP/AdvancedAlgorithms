...
class PatriciaTreeNode:
    def __init__(self, str: StringPL):
        self.str = str
...
class PatriciaTree:
    def __init__(self):
        self.root = PatriciaTreeNode(StringPL(None, 0, 0))
    ...
    def _search(self, q: str) -> (bool):
        ...
        while not cn.isLeaf():
            ...
            if cnc is not None:
                ...
                if q[sc:sc + t_l] == cnc.str[t_p:t_p+t_l]:
                    sc, cn = sc + t_l, cnc
    ...
    def insert(self, s: str):
        ...
        while cn==self.root or not cn.isLeaf():
            ...
            if cnc is not None:
                t_p, t_l = cnc.str.p, cnc.str.l
                if s[sc:sc + t_l] == cnc.str[t_p:t_p+t_l]: #case 3
                    cn,sc=cnc,sc+t_l
                else: #case 2
                    #the first char is always matched
                    for i in range(1, t_l):
                        if s[sc + i] != cnc.str[cnc.str.p + i]: break
                    cnins=PatriciaTreeNode(cnc.str.clone(cnc.str.p,i))
                    cnc.str.p,cnc.str.l=cnc.str.p+i,cnc.str.l-i
                    cn.insert(cnins)
                    cnins.insert(cnc)
                    cnleaf=PatriciaTreeNode(StringPL(s,sc+i,s_l-sc-i))
                    cnins.insert(cnleaf)
                    return
            else: #case 1
                cnleaf=PatriciaTreeNode(StringPL(s,sc,s_l-sc))
                cn.insert(cnleaf)
                return
